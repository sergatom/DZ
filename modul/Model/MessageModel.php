...
class MessageModel extends BaseModel
{
    ...
    public function save($message, $newsId, $userId)
    {
        ...
        $query = "INSERT INTO messages set `message` = '{$message['message']}', user_id = '{$userId}', news_id = '{$newsId}'";
    ...
    public function saveAnswer($message, $newsId, $userId,$responseTo)
    {
        ...
        $query = "INSERT INTO messages set `message` = '{$message['message']}', user_id = '{$userId}', news_id = '{$newsId}', response_to = '{$responseTo}'";
        return $this->db->execute($query);
    }

    public function getMessage($id)
    {
        $result = $this->db->query('select * from ' . $this->table . " LEFT JOIN login ON user_id=id left join articles ON news_id=article_id where news_id='{$id}' and response_to<1 order by rating desc");
        return $result;
    }

    public function getAnswer($newsId,$id)
    {
        $result = $this->db->query('select * from ' . $this->table . " LEFT JOIN login ON user_id=id left join articles ON news_id=article_id where news_id='{$newsId}' and response_to='{$id}' order by message_time DESC");
        return $result;
    }

    public function getMessageToAgree()
    {
        $result = $this->db->query('select * from ' . $this->table . " LEFT JOIN login ON user_id=id left join articles ON news_id=article_id where category_id='1' and is_agreed='0'");
        return $result;
    }
    ...
    public function saveAgree($id)
    {
        ...
        $query = "UPDATE messagesset set `is_agreed` = '1' where message_id='{$id}'";
        return $this->db->execute($query);
    }
    ...
    public function edit($id,$message)
    {
        ...
        $query = "UPDATE messages set `message` = '$message' where message_id='{$id}'";
        return $this->db->execute($query);
    }
    ...
    public function rating($zero, $id)
    {
        if($zero == 1)
        {
            $query = "UPDATE messages set `rating` = `rating` + '1' where message_id = '{$id}'";
        } else
            {
                  $query = "UPDATE messages set `rating` = `rating` - '1' where message_id = '{$id}'";
            }
